...
/*
 Ты IOS разработчик. У нас приложение которое находит картинки в интернете и сохраняет их на устройстве. Нам нужно сделать так чобы при нажатии на кнопку "trashBarButtonItem" у нас появлялось предупреждение "Вы действительно хотите удалить выбранные фотографии?" и при нажатии "Да", у нас удалялись выбранные фото.
 
 You are an iOS developer. We have an application that finds pictures on the Internet and saves them on the device. We need to do this so that when we click on the "trashBarButtonItem" button, we get a warning "Do you really want to delete the selected photos?" and when you click "Yes", the selected photos were deleted from us.
